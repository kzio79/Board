...
export default {
    ...
    async getContent(boardNum){
        const response =  await axios.get(`${BASE_URL}content`, {params : { boardNum: boardNum } })
        return response.data;
    },
    ...
    async postModify(boardNum, viewContent){
        const response =await axios.post(`${BASE_URL}modify`, JSON.stringify({
            boardNum: boardNum,
    ...
    async postWrite(categoryNum, writer, pw, title, content){
        const response = await axios.post(`${BASE_URL}write`, JSON.stringify({
    ...
    async postReply(boardNum, content){
        await axios.post(`${BASE_URL}content`, {
            boardNum: boardNum,
            content: content
        })
    }
